...
func testAddSCSI(u *uvm.UtilityVM, disks []string, attachOnly bool) ([]*scsi.Mount, error) {
	...
	for i := range disks {
		...
		scsiMount, err := u.SCSIManager.AddVirtualDisk(context.Background(), disks[i], false, scsi.IndividualVMAccess(u.ID()), mc)
		if err != nil {
			return nil, err
		}
	...
	for i := 0; i < maxWorkers; i++ {
		go func(scsiIndex int) {
			for {
				...
				mount, err := u.SCSIManager.AddVirtualDisk(context.Background(), path, false, scsi.IndividualVMAccess(u.ID()), nil)
				if err != nil {
					os.Remove(path)
				...
				mount, err = u.SCSIManager.AddVirtualDisk(context.Background(), path, false, scsi.IndividualVMAccess(u.ID()), &scsi.MountConfig{})
				if err != nil {
					os.Remove(path)
					t.Errorf("failed to add SCSI disk for worker: %d, iteration: %d with err: %v", scsiIndex, iteration, err)
					continue
